...
contract('UserFactory', function(accounts) {
  ...
  it('should create user with roles', () => {
    ...
    return userFactory.createUserWithProxyAndRecovery(roles, '', [], [])
    .then(result => {
      assert.equal(result.logs.length, 1);
      assert.equal(result.logs[0].event, 'UserCreated');
  ...
  it('should create users with skills', () => {
    ...
    let categories = [1, 2];
    let skills = [1];
    return userFactory.createUserWithProxyAndRecovery([], areas, categories, skills)
    .then(result => {
      assert.equal(result.logs.length, 1);
      assert.equal(result.logs[0].event, 'UserCreated');
      assert.equal(result.logs[0].args.roles.length, 0);
  ...
  it('should create users with skills and roles', () => {
    ...
    let categories = [1, 2];
    let skills = [1];
    return userFactory.createUserWithProxyAndRecovery(roles, areas, categories, skills)
    .then(result => {
      assert.equal(result.logs.length, 1);
      assert.equal(result.logs[0].event, 'UserCreated');
      assert.deepEqual(result.logs[0].args.roles, roles);
