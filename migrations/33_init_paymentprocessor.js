...
module.exports = deployer => {
    ...
    .then(() => {
        let sig = paymentGateway.contract.transferWithFee.getData(0,0,0,0,0,0).slice(0, 10);
        return rolesLibrary.addRoleCapability(PaymentProcessorRole, PaymentGateway.address, sig);
    })
    .then(() => {
        var sig = paymentGateway.contract.transferFromMany.getData([],0,[],0).slice(0, 10);
        return rolesLibrary.addRoleCapability(PaymentProcessorRole, PaymentGateway.address, sig);
    })
    .then(() => {
        let sig = paymentGateway.contract.transferToMany.getData(0,[],[],0,0,0).slice(0, 10);
        return rolesLibrary.addRoleCapability(PaymentProcessorRole, PaymentGateway.address, sig);
    })
    .then(() => {
        let sig = paymentGateway.contract.transferAll.getData(0,0,0,0,0,0,0).slice(0, 10);
        return rolesLibrary.addRoleCapability(PaymentProcessorRole, PaymentGateway.address, sig);
    })
    .then(() => {
        let sig = paymentGateway.contract.transferFromMany.getData([],0,[],0).slice(0, 10);
        return rolesLibrary.addRoleCapability(PaymentProcessorRole, PaymentGateway.address, sig);
    })
